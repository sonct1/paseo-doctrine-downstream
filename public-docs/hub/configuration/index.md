...
The generated bundle below is the stable v0.5.0 upstream shape. Paseo Foundation retains it as an
adaptation reference, but its guided command fails closed with
`HUB_FOUNDATION_ADMISSION_REQUIRED`; the shape is not a role/assignment authority receipt.

In unmodified upstream v0.5.0, `paseo hub init` and the guided continuation after interactive
`paseo hub login` write two files for the directory where they run. Paseo Foundation does neither:
