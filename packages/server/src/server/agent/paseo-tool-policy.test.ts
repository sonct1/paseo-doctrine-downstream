...
describe("Paseo tool policy", () => {
  ...
  test("denies absent and unknown tools when an allowlist is present", () => {
    const policy = { allowedTools: ["list_agents"] } satisfies ProviderPaseoToolsPolicy;

    expect(isPaseoToolEnabled(policy, "list_agents")).toBe(true);
    expect(isPaseoToolEnabled(policy, "create_agent")).toBe(false);
    expect(isPaseoToolEnabled(policy, "future_tool")).toBe(false);
