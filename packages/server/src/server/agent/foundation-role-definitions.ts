...
Every SLP repository has a target-state thin root WORKSPACE_PROTOCOL.md, including an explicit zero-delta baseline. Missing protocol is bootstrap-required: new material delegation and protected or mutating work require a valid protocol. Only an exact Human-issued bounded read-only, bootstrap, or recovery assignment may carry a reasoned, scoped, expiring exception. Invalid or unreadable protocol always fails closed.`;
