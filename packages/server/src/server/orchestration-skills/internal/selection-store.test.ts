...
import { coerceSkillSelection, createSkillSelectionStore } from "./selection-store";
...
describe("daemon agent skill selection", () => {
  it("fails malformed or unknown selection shapes back to all", () => {
    expect(coerceSkillSelection(null)).toEqual({ mode: "all" });
    expect(coerceSkillSelection({ mode: "future", skills: ["paseo"] })).toEqual({ mode: "all" });
  });

  it("normalizes custom names and drops non-string entries", () => {
    expect(
      coerceSkillSelection({ mode: "custom", skills: [" paseo ", 42, "", "paseo-loop", "paseo"] }),
    ).toEqual({ mode: "custom", skills: ["paseo", "paseo-loop"] });
  });

  it("does not retain custom names when mode is all", () => {
    expect(coerceSkillSelection({ mode: "all", skills: ["paseo"] })).toEqual({ mode: "all" });
  });
